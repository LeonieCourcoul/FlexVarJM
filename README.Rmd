...
The global function is 'lsjm'. It handles to estimate joint model
...
truncation. For more information you can read the corresponding article : https://arxiv.org/abs/2306.16785
...
This is an exemple in a simulated dataset where 'binary' is a binary variable.
...
-   with $b_i=\left(b_{0i},b_{1i}\right)^{\top}$ and
    $\tau_i=\left(\tau_{0i},\tau_{1i}\right)^{\top}$ assuming that the
    two sets of random effects $b_i$ and $\tau_i$ are not independent:
...
set of estimated parameters. To have a 'IC\%' confidence interval, the predictions are computed 'draws' time and the percentiles of the predictions are computed. For example, for the individual 15 to
